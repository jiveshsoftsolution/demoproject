...
class Staff_model extends CI_Model {
    ...
    public function get_staff_present_status_by_month($staff_id=""){
	$sql = "SELECT COUNT(attendance_id) as present_count
		FROM ems_staff_attendance
		WHERE staff_id='".$staff_id."' AND attendance_status='P' AND MONTH(attendance_date)= MONTH(CURRENT_DATE())";
	$result_data = $this->db->query($sql);
	return $result_data->result()[0];
    }
    
    public function get_staff_absent_status_by_month($staff_id=""){
	$sql = "SELECT COUNT(attendance_id) as absent_count
		FROM ems_staff_attendance
		WHERE staff_id='".$staff_id."' AND attendance_status='A' AND MONTH(attendance_date)= MONTH(CURRENT_DATE())";
	$result_data = $this->db->query($sql);
	return $result_data->result()[0];
    }
    
    public function get_staff_leave_status_by_month($staff_id=""){
	$sql = "SELECT COUNT(attendance_id) as leave_count
		FROM ems_staff_attendance
		WHERE staff_id='".$staff_id."' AND attendance_status='L' AND MONTH(attendance_date)= MONTH(CURRENT_DATE())";
	$result_data = $this->db->query($sql);
	return $result_data->result()[0];
    }
}